...
(setq tg/vertico-center-height 35
      tg/wsl-root-path         (getenv "WSL_PATH")
      tg/wsl-home-path         (when tg/wsl-root-path (expand-file-name (concat "home/" tg/user) tg/wsl-root-path))
...
(unless tg/using-windows
  (customize-set-variable 'package-user-dir (expand-file-name ".emacs.d/elpa" tg/wsl-home-path)))
(package-initialize)
(require 'use-package)
            ...
            (let ((done nil))
              (lambda (&args)
                (when (not done)
                  ...
                  (setq done t)))))
(unless (or (package-installed-p 'gnu-elpa-keyring-update) (not tg/wsl-root-path))
...
(if tg/wsl-root-path
    (progn
      ...
      (wsl-path-activate)
      (setq wslpath-exec (f-join tg/wsl-root-path "usr/bin" wslpath-exec))
      (startup-redirect-eln-cache (f-join tg/wsl-home-path ".emacs.d/eln-cache")))
...
(let* ((root-path (if tg/using-windows "C:/software/emacs/" "/usr/local/share/emacs/"))
...
;; remove carriage-returns on yank
(defun tg/insert-for-yank (orig-fun &rest args)
  (let ((coding-str (symbol-name buffer-file-coding-system)))
       (funcall orig-fun
                (if (or (string-match "utf-8-unix$" coding-str)
                        (string-match "undecided-unix$" coding-str))
